...
const MASKS = {
  GFIFOTH  : 0b10000000, // FIFO threshold            : trigger interrupt after 4 datasets in FIFO (GCONF1 <7> p.28),1 <7> p.28),),
...
const SETUP_DEFAULTS = { // During setup, (value) is written to each register (key)
  ...
  GCONF1   : MASKS.GFIFOTH, // FIFO interrupt threshold
  GCONF2   : MASKS.GGAIN | MASKS.GLDRIVE | MASKS.GWTIME, // Gesture gain, LED drive, wait time
  GOFFSET_U: 0x00, // no offset
  GOFFSET_D: 0x00, // no offset
  GOFFSET_L: 0x00, // no offset
  GOFFSET_R: 0x00, // no offset
  GPULSE   : MASKS.GPLEN | MASKS.GPULSE // pulse count and length,
};
...
const ENABLE_VALUES = { // During enable, each (value) is written to register (key)
  ...
  PPULSE : MASKS.PPLEN | MASKS.PPULSE, // Proximity pulse length and count
  CONFIG2: MASKS.LED_BOOST,
  GCONF4 : MASKS.GIEN | MASKS.GMODE,
  ENABLE : MASKS.ENABLE
};
  ...
  return (function () {
    ...
    APDS9960.prototype.readGesture = function () {
      ...
      this.io.i2cReadOnce(this.address, REGISTERS.GSTATUS, 1, status => {
        if (status & MASKS.GVALID === MASKS.GVALID) {
